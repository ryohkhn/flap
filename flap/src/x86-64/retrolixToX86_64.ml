...
module InstructionSelector : InstructionSelector =
  struct
    ...
    let mov ~(dst : dst) ~(src : src) =
      (* Most instructions (e.g., mov) permit at most
         one operand to be a memory value. *)
      match dst, src with
      | `Reg _, _ | _, `Reg _ ->
        [Instruction (movq ~src:src ~dst:dst)]
      | _ ->
        [Instruction (movq ~src:src ~dst:r15)] @
        [Instruction (movq ~src:r15 ~dst:dst)]

    (* All these instructions need to first move
       srcl to %r15 to proceed with the x86-64 instruction
       since Retrolix provides ~dst, ~srcl and ~srcr *)
    let bin ins ~dst ~srcl ~srcr =
      mov ~src:srcl ~dst:r15 @
    ...
    let div ~dst ~srcl ~srcr =
      (* dividend is stored inside %rax and need to be *)
      (* sign extendend into %rdx::%rax to proceed with idvq*)
      mov ~src:srcl ~dst:rax
      @ [Instruction cqto]
      @ mov ~src:srcr ~dst:r15
      @ [Instruction (idivq ~src:r15)]
      (* quotient is stored inside rax after idivq *)
      @ mov ~dst ~src:rax

    let andl ~dst ~srcl ~srcr =
      mov ~src:srcl ~dst:r15 @
      [Instruction (andq ~src:srcr ~dst:r15)] @
      mov ~src:r15 ~dst:dst

    let orl ~dst ~srcl ~srcr =
      mov ~src:srcl ~dst:r15 @
      [Instruction (orq ~src:srcr ~dst:r15)] @
      mov ~src:r15 ~dst:dst
    ...
    let conditional_jump ~cc ~srcl ~srcr ~ll ~lr =
      ...
      let cmp =
        ...
        | `Reg _, _ | _, `Reg _ ->
          [Instruction (cmpq ~src1:srcr ~src2:srcl)]
        | _ ->
          [Instruction (movq ~src:srcl ~dst:r15)] @
          [Instruction (cmpq ~src1:srcr ~src2:r15)]
      in
      (* jccl jumps to the label tgt if cc holds *)
      cmp @ [Instruction (jccl ~cc:cc ~tgt:ll)] @ [Instruction (jmpl ~tgt:lr)]

    let switch ?default ~discriminant ~cases () =
      let instrs, _ = Array.fold_left (fun (instrs, i) _ ->
          instrs @
          [Instruction (cmpq ~src1:discriminant ~src2:(liti i))] @
          [Instruction (jccl ~cc:E ~tgt:(Array.get cases i))],
          i + 1
        ) ([], 0) cases in
      let default_instr =
        match default with
        | Some l -> [Instruction (jmpl ~tgt:l)]
        | None -> []
      in
      instrs @ default_instr
...
module FrameManager(IS : InstructionSelector) : FrameManager =
  struct
    ...
    let frame_descriptor ~params ~locals =
      (* old %rbp and last return adress are pushed to the stack
         so arguments starts at %rbp + 16 *)
      (* locals are placed at negative values of
         base pointer since the stack grows downward *)
      let stack, _ =
        List.fold_right (fun loc (stack, offset) ->
          S.IdMap.add loc (Mint.of_int offset) stack, offset - 8
        ) locals (S.IdMap.empty, -8)
      in
      let stack, _ =
        List.fold_left (fun (stack, offset) param ->
          S.IdMap.add param (Mint.of_int offset) stack, offset + 8
        ) (stack, 16) params
      in
      { param_count = (List.length locals); locals_space = (List.length locals); stack_map = stack; }

    let location_of fd id =
      let create_addr offset base =
        let offset = Some offset in
        let base = Some base in
        let idx = None in
        T.addr ?offset:offset ?idx:idx ?base:base ()
      in
      if S.IdMap.mem id fd.stack_map then
        create_addr (T.Lit (S.IdMap.find id fd.stack_map)) (X86_64_Architecture.RBP)
      else
        create_addr (T.Lab (data_label_of_global id)) (X86_64_Architecture.RIP)

    let function_prologue fd =
      let save_rbp = [T.Instruction (T.pushq ~src:rbp)] in
      let local_rbp = IS.mov ~dst:rbp ~src:rsp in
      let allocate_locals =
        match fd.locals_space with
        | 0 -> []
        | _ -> let sub_space = T.liti (8 * fd.locals_space) in
          [T.Instruction (T.subq ~src:sub_space ~dst:rsp)]
      in
      save_rbp @ local_rbp @ allocate_locals

    let function_epilogue fd =
      let restore_rbp = [T.Instruction (T.popq ~dst:rbp)] in
      let deallocate_locals =
        match fd.locals_space with
        | 0 -> []
        | _ -> let addq_space = T.liti (8 * fd.locals_space) in
          [T.Instruction (T.addq ~src:addq_space ~dst:rsp)]
      in
      deallocate_locals @ restore_rbp

    let call fd ~kind ~f ~args =
      match kind with
      | `Normal ->
        (* in retrolix arguments to a function are used when more than 6 arguments are needed,
           so everything goes on the stack *)
        let push_args =
          List.map (fun arg -> T.Instruction (T.pushq ~src:arg)) args
          |> List.rev
        in
        (* after the call we clean the stack of the pushed arguments *)
        let deallocate_args =
        ...
        let align ins =
          [T.Instruction (ins ~src:(T.liti 8) ~dst:rsp)]
        in
        let pre_call_align, post_call_align =
        ...
        pre_call_align @
        push_args @ [T.Instruction (T.calldi ~tgt:f)] @ deallocate_args
        @ post_call_align
      | `Tail ->
        (* since we're not doing the ret after a call we need to clean the stack *)
        function_epilogue fd @
        [T.Instruction (T.jmpdi ~tgt:f)]
